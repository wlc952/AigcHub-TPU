...
class AppInitializationRouter(BaseAPIRouter):
    ...
    async def init_app(self):
        ...
        ttsmodel, tone_color_converter, g2p, lexicon = get_models()
        self.tts_models = {
            "models": ttsmodel, 
    ...
    async def destroy_app(self):
        del ttsmodel, tone_color_converter, g2p, lexicon, self.tts_models
...
async def text_to_speech(request: TTSRequest):    
    ...
    tts(request.input, request.emotion, request.voice, _name, router.tts_models['models'], router.tts_models['g2p'], router.tts_models['lexicon'])
    return Response(content=_name, media_type="text/plain")
    # save_path = _name
